...
export function buildRiskView(risk: MaintenanceRisk, index: number): PredictiveRiskView {
  ...
  return {
    ...
    confidence,
    trend,
    inspectionWindow: isHigh ? "Hari ini 16:00-18:00" : "Besok 08:00-11:00",
    status: risk.workOrderReady ? "Siap SPK" : confidence < 80 ? "Perlu validasi" : "Pantau",
    missingTelemetry: isHigh ? 4 : 13 + index * 4,
    thresholdTime: isHigh ? "11 Juli, 17.00" : "13 Juli, 09.00",
    impact: isHigh
