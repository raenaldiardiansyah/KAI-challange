    ...
    <div className="summary-grid">
      {items.map((item) => (
        <Card key={item.label} className={`summary-accent-card summary-tone-${item.tone}`}>
          <Link href={item.label === "Armada Aktif" || item.label === "Total Gerbong" ? "/trainset" : item.label === "Kesehatan Global" ? "/predictive-maintenance" : "/alarm-center"} style={{ textDecoration: 'none', color: 'inherit' }}>
