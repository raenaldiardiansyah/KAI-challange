...
export function ScaledDashboardContent({ children }: { children: ReactNode }) {
  ...
  useLayoutEffect(() => {
    ...
    const updateShellHeight = () => {
      ...
      const availableHeight = shell.parentElement?.clientHeight ?? window.innerHeight;
      shell.style.height = `${Math.max(visualContentHeight, availableHeight)}px`;
    };
  ...
  if (!shouldScale) {
    return <div className="content-scroll-area">{children}</div>;
  }
